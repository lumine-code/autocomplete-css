...
module.exports = {
  ...
  // Domain-expert tier: authoritative for CSS, and silent everywhere else.
  ...
  // paths inside the stylesheets this package exists to complete.
  ...
  getSuggestions(request) {
    ...
    const isSass =
      hasScope(scopes, "source.sass") || request.editor.getGrammar()?.scopeName === "source.sass";
    ...
    if (this.isCompletingValue(request)) {
      ...
      if (isSass && this.isCompletingSassPropertyName(request)) {
    ...
    const isCompletingTag = isSass
      ? this.isCompletingSassTagSelector(request)
      : this.isCompletingTagSelector(request);
    if (isCompletingTag) {
  ...
  triggerAutocomplete(editor) {
    return lumine.commands.dispatch(lumine.views.getView(editor), "autocomplete:activate", {
  ...
  isCompletingValue({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    const isSass =
      hasScope(scopes, "source.sass") || editor.getGrammar()?.scopeName === "source.sass";
    if (isSass && !/^\s/.test(line)) {
      return false;
    }
    ...
    return (
      ...
      this.getTextualPropertyContext({ editor, bufferPosition }) != null ||
  ...
  isCompletingName({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    const characterAtCursor = line[bufferPosition.column];
    const characterBeforeCursor = line[bufferPosition.column - 1];
    if (characterAtCursor === "{" || characterBeforeCursor === "}") {
      return false;
    }
  ...
  isCompletingSassPropertyName({ bufferPosition, editor }) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    ...
    return (
      ...
      !this.getTextualPropertyContext({
        editor,
        bufferPosition,
      })
    );
  ...
  isCompletingSassTagSelector({ editor, bufferPosition }) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    return !/^\s/.test(line) && !!this.getTagSelectorPrefix(editor, bufferPosition);
  },
  ...
  isCompletingPseudoSelector({ editor, scopeDescriptor, bufferPosition }) {
    const scopes = scopeDescriptor.getScopesArray();
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    const sassPrefix = this.getPseudoSelectorPrefix(editor, bufferPosition);
    if (
      (hasScope(scopes, "source.sass") || editor.getGrammar()?.scopeName === "source.sass") &&
      sassPrefix &&
      (!/^\s/.test(line) || /^[.#&]/.test(line.trimStart()))
    ) {
      return true;
    }
  ...
  getTextualPropertyContext({ editor, bufferPosition }) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    const match = /(?:^|[{;])\s*([a-zA-Z][a-zA-Z-]*)\s*:\s*[^;{}]*$/.exec(line);
    if (!match || !Object.hasOwn(this.properties, match[1])) {
      return null;
    }
